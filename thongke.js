    ...
    Object.keys(dailyData).forEach(dateStr => {
        ...
        if (reports.length > 0) {
            ...
            const calcDailyTotal = (startKey, endKey) => {
                const startVal = parseDisplayValue(firstMeters[startKey]);
                const endVal = parseDisplayValue(lastMeters[endKey]);
            ...
            reports.forEach(report => {
                ...
                (report.chemicals || []).forEach(chem => {
                    ...
                    const qty = parseDisplayValue(chem.quantity) || 0;
                    if (name && qty > 0) {
                ...
                if (parseDisplayValue(report.meters?.flow_in_start) === 0 || parseDisplayValue(report.meters?.flow_in_end) === 0) {
                ...
                const r_flow_in = (parseDisplayValue(m.flow_in_end) - parseDisplayValue(m.flow_in_start));
                const r_flow_out = (parseDisplayValue(m.flow_out_end) - parseDisplayValue(m.flow_out_start));
                const r_nuoc = (parseDisplayValue(m.nuoc_cuoi) - parseDisplayValue(m.nuoc_dau));
                ...
                const r_dien_bt = (parseDisplayValue(m.dien_bt_cuoi) - parseDisplayValue(m.dien_bt_dau));
                const r_dien_cd = (parseDisplayValue(m.dien_cd_cuoi) - parseDisplayValue(m.dien_cd_dau));
                const r_dien_td = (parseDisplayValue(m.dien_td_cuoi) - parseDisplayValue(m.dien_td_dau));
...
function calculateSummary(periodReports, periodStartDateStr, periodEndDateStr) {
    ...
    // Tránh kiểm tra các ngày trong tương lai hoặc hôm nay chưa kết thúc
    const effectiveEndDateStr = periodEndDateStr > yesterdayStr ? yesterdayStr : periodEndDateStr;

    if (periodStartDateStr <= effectiveEndDateStr) {
        const hasReportOnStartDate = periodReports.some(r => r.reportDate === periodStartDateStr);
        const hasReportOnEndDate = periodReports.some(r => r.reportDate === effectiveEndDateStr);
        if (!hasReportOnStartDate || !hasReportOnEndDate) {
            isInaccurate = true;
            console.warn("Missing reports on start or end date of the period.");
        }
    } else {
        // Nếu kỳ lọc bắt đầu hoàn toàn trong tương lai
        isInaccurate = true;
    }
...
function renderCustomPeriod(summaryData, isCustomPeriod) {
    ...
    // Hàm cập nhật ô (Cập nhật song song cả mobile)
    const updateCell = (id, value, fractionDigits = 1, forceDecimals = false) => {
        ...
        const mobileCell = document.getElementById('mobile-' + id);
        if (mobileCell) {
            mobileCell.textContent = formatNumberForDisplay(value, fractionDigits, forceDecimals);
        }
        if (!cell && !mobileCell) {
    ...
    // Cập nhật hóa chất khác trên Mobile
    const mobileOtherContainer = document.getElementById('mobile-other-chemicals-container');
    if (mobileOtherContainer) {
        mobileOtherContainer.innerHTML = '';
        let mobileOtherHtml = '';
        sortedChemNames.forEach(name => {
            if (name !== 'totalAll' && !standardChemicals.includes(name)) {
                const displayName = name.charAt(0).toUpperCase() + name.slice(1);
                mobileOtherHtml += `
                    <div class="chem-sub-card">
                      <div class="chem-sub-header">
                        <span class="chem-sub-name">${displayName}</span>
                      </div>
                      <div class="chem-sub-body">`;
                periods.forEach(period => {
                    const isHidden = (period === 'ky' && !isCustomPeriod) || (period !== 'ky' && isCustomPeriod);
                    const periodLabel = period === 'tuan' ? 'Tuần' : (period === 'thang' ? 'Tháng' : (period === 'nam' ? 'Năm' : 'Kỳ'));

                    const totalVal = summaryData.chemicals[name]?.['total'] ?? null;
                    const avgVal = summaryData.chemicals[name]?.['avg'] ?? null;
                    const displayTotal = formatNumberForDisplay(totalVal, 1, false);
                    const displayAvg = formatNumberForDisplay(avgVal, 1, true);

                    mobileOtherHtml += `
                        <div class="chem-sub-row" id="mobile-${name}-row-${period}" style="${isHidden ? 'display:none;' : ''}">
                          <span class="chem-sub-label">${periodLabel}:</span>
                          <div>
                            <span class="chem-sub-val">${displayTotal}</span><span class="chem-sub-unit"> Kg</span>
                            <span class="chem-sub-sep">/</span>
                            <span class="chem-sub-avg">${displayAvg}</span><span class="chem-sub-unit"> Kg/ngày</span>
                          </div>
                        </div>`;
                });
                mobileOtherHtml += `
        ...
        mobileOtherContainer.innerHTML = mobileOtherHtml;
    }

    // --- 3. CẬP NHẬT GHI CHÚ ---
    let notesList = [...summaryData.notes];
    if (summaryData.isInaccurate) {
        notesList.unshift("Dữ liệu có thể thiếu chính xác do thiếu báo cáo.");
    }
    const notesHtml = notesList.length > 0 ? notesList.join('<br>') : '';
    ...
    // Cập nhật ghi chú trên Mobile
    const mobileNotesCard = document.getElementById('mobile-notes-card');
    const mobileNotesContent = document.getElementById('mobile-notes-content');
    if (mobileNotesCard && mobileNotesContent) {
        if (notesHtml) {
            mobileNotesContent.innerHTML = notesHtml;
            mobileNotesCard.style.display = 'block';
        } else {
            mobileNotesCard.style.display = 'none';
        }
    }

    // --- 5. HIỂN THỊ/ẨN CẢNH BÁO ---
    if (accuracyWarningDiv) {
        accuracyWarningDiv.style.display = 'none';
    }
...
function renderSummary(weekData, monthData, yearData) {
    ...
    if (mobileOtherContainer) {
        ...
        sortedChemNames.forEach(name => {
            ...
            periods.forEach(period => {
                const periodData = dataMap[period];
                const totalVal = periodData?.chemicals[name]?.['total'] ?? null;
                const avgVal = periodData?.chemicals[name]?.['avg'] ?? null;
                const displayTotal = formatNumberForDisplay(totalVal, 1, false);
                const displayAvg = formatNumberForDisplay(avgVal, 1, true);
                const periodLabel = period === 'tuan' ? 'Tuần' : (period === 'thang' ? 'Tháng' : 'Năm');

                mobileOtherHtml += `
                    <div class="chem-sub-row" id="mobile-${name}-row-${period}">
                      <span class="chem-sub-label">${periodLabel}:</span>
                      <div>
                        <span class="chem-sub-val">${displayTotal}</span><span class="chem-sub-unit"> Kg</span>
                        <span class="chem-sub-sep">/</span>
            ...
            // Dòng 'ky' ẩn mặc định
            mobileOtherHtml += `
                    <div class="chem-sub-row" id="mobile-${name}-row-ky" style="display:none;">
                      <span class="chem-sub-label">Kỳ:</span>
                      <div>
                        <span class="chem-sub-val">--</span><span class="chem-sub-unit"> Kg</span>
                        <span class="chem-sub-sep">/</span>
                        <span class="chem-sub-avg">--</span><span class="chem-sub-unit"> Kg/ngày</span>
                      </div>
                    </div>`;
    ...
    let notesList = yearData?.notes ? [...yearData.notes] : [];
    if (isInaccurate) {
        notesList.unshift("Dữ liệu có thể thiếu chính xác do thiếu báo cáo.");
    }
    const notesHtml = notesList.length > 0 ? notesList.join('<br>') : '';
    ...
    // Cập nhật ghi chú trên Mobile/Unified Dashboard
    const mobileNotesCard = document.getElementById('mobile-notes-card');
    const mobileNotesContent = document.getElementById('mobile-notes-content');
    if (mobileNotesCard && mobileNotesContent) {
        if (notesHtml) {
    ...
    document.querySelectorAll('[id^="col-week"], [id*="-tong-tuan"]:not([id^="mobile-"]), [id*="-tb-tuan"]:not([id^="mobile-"])').forEach(el => el.style.display = '');
    document.querySelectorAll('[id^="col-month"], [id*="-tong-thang"]:not([id^="mobile-"]), [id*="-tb-thang"]:not([id^="mobile-"])').forEach(el => el.style.display = '');
    document.querySelectorAll('[id^="col-year"], [id*="-tong-nam"]:not([id^="mobile-"]), [id*="-tb-nam"]:not([id^="mobile-"])').forEach(el => el.style.display = '');
    document.querySelectorAll('[id^="col-period"], [id*="-tong-ky"]:not([id^="mobile-"]), [id*="-tb-ky"]:not([id^="mobile-"])').forEach(el => el.style.display = 'none');
...
function calculateAndRenderSummary(periodType = 'year', customStartDate = null, customEndDate = null) {
    ...
    document.querySelectorAll('[id^="col-week"], [id*="-tong-tuan"]:not([id^="mobile-"]), [id*="-tb-tuan"]:not([id^="mobile-"])').forEach(el => el.style.display = 'none');
    document.querySelectorAll('[id^="col-month"], [id*="-tong-thang"]:not([id^="mobile-"]), [id*="-tb-thang"]:not([id^="mobile-"])').forEach(el => el.style.display = 'none');
    document.querySelectorAll('[id^="col-year"], [id*="-tong-nam"]:not([id^="mobile-"]), [id*="-tb-nam"]:not([id^="mobile-"])').forEach(el => el.style.display = 'none');
    document.querySelectorAll('[id^="col-period"], [id*="-tong-ky"]:not([id^="mobile-"]), [id*="-tb-ky"]:not([id^="mobile-"])').forEach(el => el.style.display = '');

    // Cập nhật hiển thị dòng trên Mobile (Ẩn Tuần/Tháng/Năm, hiện Kỳ)
    document.querySelectorAll('[id$="-row-tuan"], [id$="-row-thang"], [id$="-row-nam"]').forEach(el => el.style.display = 'none');
    document.querySelectorAll('[id$="-row-ky"]').forEach(el => el.style.display = '');
